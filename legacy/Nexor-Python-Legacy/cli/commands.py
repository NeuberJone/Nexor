...
def print_available_job_line(job) -> None:
    print(
        ...
        f"Effective={format_meters(job.actual_printed_length_m)}m | "
        f"Gap={format_meters(job.gap_before_m)}m | "
        f"Consumed={format_meters(job.consumed_length_m)}m | "
        f"Doc={job.document}"
    )
...
def handle_list_jobs(
    *,
    ...
    print("\nNEXOR AVAILABLE JOBS\n")

    repo = ProductionRepository()
    jobs = repo.list_available_jobs(
        machine=machine,
        fabric=fabric,
    ...
    if not jobs:
        print("Nenhum job disponível para montagem de rolo.")
        return 0

    print(f"Jobs disponíveis: {len(jobs)}\n")
    for job in jobs:
        print_available_job_line(job)

    return 0


def handle_create_roll(
    *,
    machine: str,
    fabric: str | None = None,
    note: str | None = None,
    ...
    print("Status: CRIADO")
    print(f"ID: {roll_id}")
    print(f"Roll: {roll.roll_name if roll else '-'}")
    print(f"Machine: {roll.machine if roll else machine}")
    print(f"Fabric: {roll.fabric if roll else (fabric or '-')}")
    print(f"Status atual: {roll.status if roll else '-'}")
    return 0


def handle_add_job_to_roll(
    *,
    roll_id: int,
    job_row_id: int,
) -> int:
    print("\nNEXOR ADD JOB TO ROLL\n")
    print(f"Roll ID: {roll_id}")
    print(f"Job row ID: {job_row_id}\n")

    repo = ProductionRepository()

    job = repo.get_available_job_by_row_id(job_row_id)
    if not job:
        if repo.job_is_assigned_to_any_roll(job_row_id):
            print("Status: ERRO")
            print("Motivo: Este job já está atribuído a um rolo.")
            return 1

        print("Status: ERRO")
        print(f"Motivo: Job não disponível para montagem: id={job_row_id}")
        return 1

    try:
        repo.add_job_to_roll(
            roll_id=roll_id,
...
def handle_remove_job_from_roll(
    *,
    roll_id: int,
    job_row_id: int,
) -> int:
    print("\nNEXOR REMOVE JOB FROM ROLL\n")
    print(f"Roll ID: {roll_id}")
    print(f"Job row ID: {job_row_id}\n")
    ...
    if removed <= 0:
        print("Status: ERRO")
        print("Motivo: Nenhum item removido.")
        return 1

    print("Status: REMOVIDO")

    summary = repo.get_roll_summary(roll_id)
    print("\nResumo do rolo após remoção:")
    print(f"Jobs: {summary['jobs_count']}")
    print(f"Total planned (m): {format_meters(summary['total_planned_m'])}")
    print(f"Total effective (m): {format_meters(summary['total_effective_m'])}")
    print(f"Total gap (m): {format_meters(summary['total_gap_m'])}")
...
def handle_close_roll(
    *,
    ...
    try:
        repo.close_roll(roll_id=roll_id, note=note)
    except Exception as exc:
        print("Status: ERRO")
        print(f"Motivo: {exc}")
        return 1
