...
def map_sections_to_job(sections: dict, source_path: str | None = None) -> ProductionJob:
    ...
    consumed_height_mm = parse_float(costs.get("PrintHeightMM"))

    if consumed_height_mm == 0 and planned_height_mm > 0:
        consumed_height_mm = planned_height_mm

    vpos_mm = parse_float(item.get("VPosMM") or item.get("VPositionMM"))

    planned_length_m = planned_height_mm / 1000
    consumed_length_m = consumed_height_mm / 1000
